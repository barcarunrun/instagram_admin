  ...
  function retryJob(jobId: string) {
    startTransition(async () => {
      try {
        ...
        setLogs((current) =>
          current.map((log) => (log.id === jobId ? updated : log)),
        );
    ...
    <section className="table-card">
      <div className="panel-head">
        <div>
          <h3>投稿実行ログ</h3>
          <p className="muted">
            成功、失敗、再試行履歴とエラー詳細を確認できます。
          </p>
        </div>
        {message ? (
          <p
            className={message.includes("失敗") ? "error-text" : "success-text"}
          >
            {message}
          </p>
        ) : null}
          ...
          <article key={log.id} className="log-item">
            <div className="log-item-head">
              <div>
                ...
                <p style={{ marginTop: 8 }}>
                  {log.errorMessage ?? "投稿が完了しました。"}
                </p>
                <p className="muted" style={{ marginTop: 6 }}>
                  {new Date(log.executedAt).toLocaleString("ja-JP")} / retry:{" "}
                  {log.retryCount}
                </p>
              </div>
              <button
                className="ghost-button"
                onClick={() => retryJob(log.id)}
                disabled={isPending}
              >
                再実行する
              </button>
