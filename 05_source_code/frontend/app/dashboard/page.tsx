    ...
    <AppShell currentPath="/dashboard">
      ...
      <form className="filter-bar" method="get">
        ...
        <div className="filter-bar-actions">
          ...
          <Link className="secondary-button" href={`/calendar?from=${range.from}&to=${range.to}&view=week`}>
            カレンダーで確認
          </Link>
        </div>
      </form>
      ...
      <section className="stats-grid">
        ...
        <article className="card stat-card">
          ...
          <div className="stat-change negative">失敗または要対応の対象を監視</div>
        </article>
        <article className="card stat-card">
          <div className="stat-head">
            <span>未実行件数</span>
      ...
      <section className="dashboard-grid">
        ...
        <article className="list-panel">
          ...
          <div className="failure-list">
            {failureItems.length > 0 ? failureItems.map(({ log, content }, index) => (
      ...
      <section className="dashboard-grid dashboard-grid-bottom">
        <article className="list-panel">
          ...
          <div className="alert-list">
            {summary.unexecuted.length > 0 ? summary.unexecuted.map((item) => {
              const content = contents.items.find((entry) => entry.id === item.contentId);

              return (
                <div key={item.id} className="alert-item">
                  <div className="failure-item-head">
                    <div>
                      <div className="failure-title">{content?.title ?? "予約投稿"}</div>
                      <div className="failure-meta">
                        {new Date(item.publishAt).toLocaleString("ja-JP")} ・ {item.timezone}
                      </div>
                    </div>
        ...
        <article className="table-card">
          ...
          <div>
            ...
            <p className="muted">再認可対象と KPI 未達アラートを表示します。</p>
          </div>
          ...
          {summary.reauthorizationAccounts.length === 0 && summary.alerts.length === 0 ? (
