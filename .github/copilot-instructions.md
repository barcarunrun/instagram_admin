...
- TASK-ID を完了として扱う前に、対象ブランチを push して .github/workflows/ci.yml の CI を必ず実行する。
- CI が失敗している状態ではタスク完了扱いにしない。必要なら PR を作成または更新し、CI の結果まで確認する。
...
- タスク完了時は、少なくとも変更対象サービスの typecheck / lint / format:check を通し、push または PR 更新で GitHub Actions の CI を起動する。
