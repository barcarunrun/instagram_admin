...
| TASK-003 から TASK-006 | 実装済み   | 一部実装   | 確認済み | モック、認証、OAuth 導線は実装済み。連携導線の確認は TASK-007 の E2E が一部代替。GitHub Actions で backend、frontend、worker と e2e-connect の成功を確認済み。                                                                                             |
| TASK-007               | 実装済み   | 実装済み   | 確認済み | [05_source_code/frontend/e2e/connect-workflow.spec.ts](05_source_code/frontend/e2e/connect-workflow.spec.ts) で連携成功、期限切れ、再認可復旧を確認可能。ローカル実行と GitHub Actions の両方で成功を確認済み。                                           |
...
| TASK-019 から TASK-024 | 実装済み   | 実装済み   | 未確認   | 正規化層、カルーセル順序、リール拡張、種別切替 UI、受入テストを追加済み。backend の型検査 / lint / test、frontend の typecheck / lint、Playwright の content-workflow spec 成功をローカルで確認済み。                                                                 |
| TASK-025               | 実装済み   | 実装済み   | 未確認   | `/schedules/validate` に未来日時、連携状態、重複予約、承認待ち検証あり。Playwright で過去日時エラーを確認済み。                                                                                                                                             |
| TASK-026               | 実装済み   | 実装済み   | 未確認   | 予約作成に加えて `GET/PUT/DELETE /schedules` と `GET /schedules/content/:contentId` を追加済み。Playwright で更新・取消導線を確認済み。                                                                                                                   |
| TASK-027 から TASK-028 | 実装済み   | 実装済み   | 未確認   | 重複防止制約と予約 UI 導線は実装済み。Playwright で重複予約拒否と未来時刻予約を確認済み。                                                                                                                                                                   |
| TASK-029               | 実装済み   | 実装済み   | 未確認   | 予約確認、変更、取消をコンテンツ編集画面から実行可能。Playwright で更新・取消を確認済み。                                                                                                                                                                   |
| TASK-030               | 実装済み   | 未着手     | 未確認   | `/calendar/events` API は実装済み。                                                                                                                                                                                                                         |
| TASK-031               | 実装済み   | 実装済み   | 未確認   | [05_source_code/frontend/e2e/content-workflow.spec.ts](05_source_code/frontend/e2e/content-workflow.spec.ts) に TC-301 から TC-305 を追加し、未来時刻予約、過去日時エラー、重複予約、予約取消、予約更新を自動化済み。                                       |
| TASK-032 から TASK-037 | 一部実装   | 未着手     | 未確認   | Redis キュー投入、Scheduler 雛形、Worker 雛形、internal job execution API、状態遷移の基本分岐、local mock の一時障害シナリオを追加。E2E と CI は未確認。                                                                                                   |
| TASK-038 から TASK-039 | 実装済み   | 未着手     | 未確認   | 再実行 API とログ UI は実装済み。`/jobs/:jobId/retry` は Redis キューへの再投入まで接続済み。ログ UI は [05_source_code/frontend/app/logs/page.tsx](05_source_code/frontend/app/logs/page.tsx) と [05_source_code/frontend/components/job-log-panel.tsx](05_source_code/frontend/components/job-log-panel.tsx) で確認。 |
| TASK-040               | 未着手     | 未着手     | 未確認   | 障害回復シナリオの受入テストは未追加。                                                                                                                                                                                                                      |
| TASK-041               | 実装済み   | 未着手     | 確認済み | KPI 集計を未実行件数を含む定義へ補正済み。                                                                                                                                                                                                                  |
| TASK-042               | 実装済み   | 未着手     | 確認済み | `/dashboard/summary` を追加し、`/dashboard/kpi` `alerts` `calendar/events` も期間指定対応済み。                                                                                                                                                            |
| TASK-043               | 実装済み   | 未着手     | 確認済み | ダッシュボード UI を動的データへ接続し、失敗一覧、未実行一覧、再認可一覧を表示可能。                                                                                                                                                                        |
| TASK-044               | 実装済み   | 未着手     | 確認済み | 投稿カレンダー画面を追加し、Day / Week / Month 切替と状態表示を実装済み。                                                                                                                                                                                  |
| TASK-045               | 実装済み   | 未着手     | 確認済み | KPI 未達アラートを dashboard alert に統合済み。                                                                                                                                                                                                             |
| TASK-046               | 実装済み   | 実装済み   | 確認済み | [05_source_code/frontend/e2e/operations-visibility.spec.ts](05_source_code/frontend/e2e/operations-visibility.spec.ts) で KPI 表示、カレンダー表示、失敗投稿識別、未達アラート、再認可対象表示を自動化済み。                                                |
| TASK-047 から TASK-051 | 未着手     | 未着手     | 未確認   | 承認フローと関連受入テストは未確認。                                                                                                                                                                                                                        |
...
3. **TASK-019**: 標準ペイロード正規化層実装
   ...
   - 補足: 種別別の `contentConfig` とメディア情報から Instagram Graph API 向け canonical payload を生成する `content-normalization.ts` を実装済み。
   - ステータス: 実装済み（2026-06-20、CI未確認）
   - ストーリーポイント: 5
   - 担当者: Backend
...
4. **TASK-020**: カルーセル順序管理実装
   ...
   - 補足: `contentConfig.orderedMediaAssetIds` と `content_media_assets.display_order` を使った順序保持、および個別メディア検証結果の表示を実装済み。
   - ステータス: 実装済み（2026-06-20、CI未確認）
   - ストーリーポイント: 3
   - 担当者: Backend
   - 依存タスク: TASK-018
...
5. **TASK-021**: リール拡張設定実装
   ...
   - 補足: `contentConfig.coverAssetId` によるカバー画像保存と、リール保存時の必須検証を実装済み。
   - ステータス: 実装済み（2026-06-20、CI未確認）
   - ストーリーポイント: 3
   - 担当者: Backend
   - 依存タスク: TASK-018
...
6. **TASK-022**: 新投稿種別追加用の設定駆動拡張ポイント実装
   ...
   - 補足: `contentTypeRules` と `contentConfig` を軸に、extension 種別を含む設定駆動の拡張ポイントを実装済み。
   - ステータス: 実装済み（2026-06-20、CI未確認）
   - ストーリーポイント: 5
   - 担当者: Backend
   - 依存タスク: TASK-019
...
7. **TASK-023**: 種別切替UI実装
   ...
   - 補足: 種別別ヒント、カルーセル順序 UI、リールカバー画像選択、拡張テンプレート入力、再検証表示を実装済み。
   - ステータス: 実装済み（2026-06-20、CI未確認）
   - ストーリーポイント: 5
   - 担当者: Frontend
...
8. **TASK-024**: 投稿種別対応受入テスト実装
   ...
   - 補足: `content-workflow.spec.ts` にカルーセル順序、リールカバー必須、拡張種別テンプレート保持を追加し、関連テスト文書も更新済み。
   - ステータス: 実装済み（2026-06-20、CI未確認）
   - ストーリーポイント: 3
   - 担当者: QA
   - 依存タスク: TASK-023
...
2. **TASK-026**: 予約登録・更新・取消API実装
   ...
   - 補足: 予約作成時の `schedules` / `posting_jobs` 生成に加えて、`GET /schedules/content/:contentId`、`GET /schedules/:id`、`PUT /schedules/:id`、`DELETE /schedules/:id` を実装済み。取消は scheduled と failed を対象にし、公開結果が存在する予約は拒否する。
   - ステータス: 実装済み（2026-06-20、CI未確認）
   - ストーリーポイント: 5
   - 担当者: Backend
   - 依存タスク: TASK-025
...
5. **TASK-029**: 予約確認・編集導線UI実装
   ...
   - 補足: コンテンツ編集画面で現在の予約情報表示、日時更新、取消確認ダイアログを実装済み。
   - ステータス: 実装済み（2026-06-20、CI未確認）
   - ストーリーポイント: 3
   - 担当者: Frontend
   - 依存タスク: TASK-028
...
7. **TASK-031**: 予約投稿受入テスト実装
   ...
   - 補足: Playwright による実行可能テストを `05_source_code/frontend/e2e/content-workflow.spec.ts` へ追加し、未来時刻予約、過去日時エラー、重複予約、予約取消、予約更新シナリオを確認済み。
   - ステータス: 実装済み（2026-06-20、CI未確認）
   - ストーリーポイント: 3
   - 担当者: QA
...
1. **TASK-032**: Scheduler実装
   ...
   - 補足: `backend/src/scheduler.ts` と Redis queue helper を追加し、due job の claim と queue 投入の雛形を実装済み。
   - ステータス: 一部実装（2026-06-20、CI未確認）
   - ストーリーポイント: 5
   - 担当者: Backend
...
2. **TASK-033**: 投稿Worker実装
   ...
   - 補足: Redis consumer と backend internal API を使う Worker 実装を追加し、local mock publish の成功 / 失敗応答を処理可能にした。
   - ステータス: 一部実装（2026-06-20、CI未確認）
   - ストーリーポイント: 5
   - 担当者: Backend
...
3. **TASK-034**: 実行状態遷移モデル実装
   ...
   - 補足: `start / complete / fail` の internal job API を追加し、scheduled / retrying / running / success / failed / action_required / reauthorization_required の基本遷移を実装済み。
   - ステータス: 一部実装（2026-06-20、CI未確認）
   - ストーリーポイント: 5
   - 担当者: Backend
   - 依存タスク: TASK-033
...
4. **TASK-035**: エラー分類ロジック実装
   ...
   - 補足: AUTH_EXPIRED、PERMISSION_DENIED / VALIDATION_ERROR、RATE_LIMIT、TIMEOUT / NETWORK_ERROR / UNKNOWN_RESULT の基本分類を backend 側へ実装済み。
   - ステータス: 一部実装（2026-06-20、CI未確認）
   - ストーリーポイント: 5
   - 担当者: Backend
   - 依存タスク: TASK-033
...
5. **TASK-036**: 自動再試行ポリシー実装
   ...
   - 補足: 指数バックオフ、再試行上限、Worker の accountId 単位 lease による最小の並列抑止を実装済み。
   - ステータス: 一部実装（2026-06-20、CI未確認）
   - ストーリーポイント: 5
   - 担当者: Backend
   - 依存タスク: TASK-035
...
6. **TASK-037**: 不明状態再照会・冪等処理実装
   ...
   - 補足: local mock に `unknown_result` と publish-status 再照会を追加し、publish result の二重保存防止を実装済み。外部 API 実装と E2E は未確認。
   - ステータス: 一部実装（2026-06-20、CI未確認）
   - ストーリーポイント: 5
   - 担当者: Backend
...
7. **TASK-038**: 手動再実行API実装
   ...
   - 補足: `/jobs/:jobId/retry` で再実行受付と監査ログ記録を実装済み。2026-06-20 時点で Redis キューへの再投入まで接続済み。
   - ステータス: 実装済み（2026-06-20、CI未確認）
   - ストーリーポイント: 3
   - 担当者: Backend
