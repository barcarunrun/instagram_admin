...
1. **TASK-005**: Facebook OAuth認可フロー実装
   - 説明: 認可開始、コールバック、state 検証、失敗時ハンドリングを含む OAuth フローを実装し、ローカル環境でコールバック確認できるようにする。
   - ストーリーポイント: 5
   - 担当者: Backend
   - 依存タスク: TASK-004
   - 関連PR: 未定

2. **TASK-006**: 連携対象アカウント選択API実装
   - 説明: FacebookページとInstagramビジネスアカウント候補を取得し、ユーザーが選択できるAPIを実装する。
   - ストーリーポイント: 3
   - 担当者: Backend
   - 依存タスク: TASK-005
   - 関連PR: 未定

3. **TASK-007**: 連携状態・権限状態保存モデル実装
   - 説明: アカウントID、ページID、権限状態、トークン有効期限、最終検証結果を保存するデータモデルを実装する。
   - ストーリーポイント: 3
   - 担当者: Backend
   - 依存タスク: TASK-005
   - 関連PR: 未定

4. **TASK-008**: 投稿権限チェックAPI実装
   - 説明: 連携完了時に必要権限の有無を検証し、未充足時は理由を返す API を実装する。
   - ストーリーポイント: 3
   - 担当者: Backend
