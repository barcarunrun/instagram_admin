...
| TASK-008 から TASK-010 | 実装済み   | 一部実装   | 未確認   | `/media-assets` upload API、種別別メディア検証、storage_key ハッシュによる重複保存抑止を追加済み。Docker daemon 停止中のため実動確認と CI は未実施。                                                                                                        |
| TASK-011 から TASK-015 | 実装済み   | 一部実装   | 未確認   | 下書き CRUD、複製、履歴、一覧 UI、編集 UI は実装済み。upload API 接続を追加し、受入テストは TASK-016 で追加済みだが未実行。                                                                                                                                 |
| TASK-016               | 実装済み   | 実装済み   | 未確認   | [05_source_code/frontend/e2e/content-workflow.spec.ts](05_source_code/frontend/e2e/content-workflow.spec.ts) に画像、動画、メディア不正、複製、履歴保存シナリオを追加。Docker daemon 停止中のため未実行。                                                   |
