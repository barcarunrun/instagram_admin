...
| TASK-008 から TASK-010 | 実装済み   | 一部実装   | 未確認   | `/media-assets` upload API、種別別メディア検証、storage_key ハッシュによる重複保存抑止を追加済み。ローカルで upload、重複保存抑止、file endpoint 配信を確認済み。CI は未実施。                                                                                |
| TASK-011 から TASK-015 | 実装済み   | 一部実装   | 未確認   | 下書き CRUD、複製、履歴、一覧 UI、編集 UI は実装済み。upload API 接続を追加し、TASK-016 の Playwright をローカル実行して主要導線を確認済み。CI は未実施。                                                                                                     |
| TASK-016               | 実装済み   | 実装済み   | 未確認   | [05_source_code/frontend/e2e/content-workflow.spec.ts](05_source_code/frontend/e2e/content-workflow.spec.ts) に画像、動画、メディア不正、複製、履歴保存シナリオを追加。ローカル実行で成功を確認済み。CI は未実施。                                         |
